...
(function () {
  ...
  * Version 1.7
  ...
  const oldSpriteIni1 = Sprite.prototype.initialize;
  ...
  const oldSpriteUpdate1 = Sprite.prototype.update;
  ...
  * Version 2.0
  ...
  const oldWB_initialize1 = Window_Base.prototype.initialize;
  ...
  const oldWB_update1 = Window_Base.prototype.update;
  ...
  Window_Base.prototype.leU_updateFloat = function () {
    ...
    const data = this._leU_floatData || undefined;
    if (!data) return;
    //- X
    let fx = '';
    if (data.sens[0] === "+") {
      fx = this.x + data.speed;
    }
    else {
      fx = this.x - data.speed;
    }
  ...
  const oldTI_onMouseMove = TouchInput._onMouseMove;
...
LeUtilities.findBattlerSprite = battler => {
...
LeUtilities.getScene = () => SceneManager._scene;

LeUtilities.isScene = function (str = "Scene_Battle") {
  if (str !== "Scene_Battle") return false;
  return this.getScene().constructor.name === str;
};
...
LeUtilities.stringAppendWithSym = (str, toAppend, sym) => {
...
LeUtilities.stringSplit = (str, sym) => {
  str = String(str);
  if (str === "") {
    return [];
  }
...
LeUtilities.CommandGetTextAsArg = (args, start) => {
  var text = "";
...
LeUtilities.getRandomValueInArray = array => {
...
LeUtilities.removeInArray = (array, element) => {
...
LeUtilities.removeAllChildren = holder => {
...
LeUtilities.rectRectCollision = (rect1, rect2) => !(rect2.leU_left() > rect1.leU_right() ||
...
LeUtilities.isMyFriend = (battler, test) => (battler.isActor() && test.isActor()) || (!battler.isActor() && !test.isActor());
...
LeUtilities.uniqArray = array => {
  /*var holder = {};
  return array.filter(function(item) {
      return holder.hasOwnProperty(item) ? false : (holder[item] = true);
  });*/
  const arr = (value, index, self) => self.indexOf(value) === index;
  return array.filter(arr);
};

LeUtilities.AToBRelation = (x, x1, x2, y1, y2, s = 1.0) => ((y2 - y1) / ((x2 - x1) ** s) * 1.0) * ((x - x1) ** s) + y1;
...
LeUtilities.cloneArray = array => array.slice(0);
...
LeU_WindowConfirmation.prototype.maxCols = () => 2;
...
LeUtilities.getMinInArray = function (array) {
  return array.sort((a, b) => (a > b) ? 1 : ((a < b) ? -1 : 0))[0];
};

LeUtilities.getMaxInArray = function (array) {
  return array.sort((a, b) => (a > b) ? 1 : ((a < b) ? -1 : 0)).pop();
};

LeUtilities.distanceBetween = (a, b) => {
  var dx = a.x - b.x;
  var dy = a.y - b.y;
  return Math.sqrt(dx * dx + dy * dy);
};

LeUtilities.distanceBetweenCells = (a, b) => Math.abs(a.x - b.x) + Math.abs(a.y - b.y);

LeUtilities.sortByClosestDistance = (obj, array, prop) => array.sort((a, b) => {
  let obj_aDist = LeUtilities.distanceBetween(prop ? prop(a) : a, obj);
  var obj_bDist = LeUtilities.distanceBetween(prop ? prop(b) : b, obj);
  return obj_aDist - obj_bDist;
});

LeUtilities.sortByFarthestDistance = (obj, array, prop) => array.sort((a, b) => {
...
LeUtilities.getPixelsOfLine = (x0, y0, x1, y1, sprite) => {
  var pixels = [];
...
LeUtilities.doesRectIncludeCoord = (cx, cy, w, h, coords) => {
  var x = coords[0];
  var y = coords[1];
...
LeUtilities.isNumeric = n => !isNaN(parseFloat(n)) && isFinite(n);
...
LeUtilities.randValueBetween = (min, max) => Math.floor(Math.random() * (max - min + 1) + min);
...
LeU_WindowScrollable.prototype.visibleLines = () => 1;
...
LeU_WindowScrollable.prototype.drawConcents = () => {
...
LeUtilities.getWrappedLines = (text, window) => {
...
LeUtilities.shrinkTextWithUnderscores = (text, lower) => text.replace(/\_(.)/ig, word => word[1].toUpperCase()).replace(/\b(.)/i, word => {
...
* Version 2.6
-------------------------------------------------------------------------*/
LeUtilities.directionCodeToText = dirCode => {
  switch (dirCode) {
    case 2:
      return "down";
    case 4:
...
LeUtilities.directionTextToCode = dirText => {
  switch (dirText) {
    case "down":
      return 2;
    case "left":
...
LeUtilities.getMinInArrayBy = function (array, prop) {
  return array.sort((a, b) => (a[prop] > b[prop]) ? 1 : ((a[prop] < b[prop]) ? -1 : 0))[0];
};

LeUtilities.getMaxInArrayBy = function (array, prop) {
  return array.sort((a, b) => (a[prop] > b[prop]) ? 1 : ((a[prop] < b[prop]) ? -1 : 0)).pop();
};

LeUtilities.stringToArray = str => {
  if (!str) return [];
  str = str.replace(/ /ig, "").replace(/\,$/, "");
  return str.replace(/ /ig, "").split(",").map(data => {
    if (isNaN(Number(data.trim()))) return String(data).trim();
    return Number(data.trim());
  });
};

LeUtilities.shuffleArray = a => {
  var j;
  var x;
  var i;
...
LeUtilities.doesResourceExist = path => {
  var bitmap = Bitmap.load(path);
  return !bitmap.isError();
};

LeUtilities.fastEase = (time, step, func) => {
...
LeUtilities.isAlly = (user, battler) => user.isActor() && battler.isActor() || user.isEnemy() && battler.isEnemy();

LeUtilities.isEnemy = function (user, battler) {
  return !this.isAlly(user, battler);
};

LeUtilities.perfectStringList = str => {
  while (str.match(/\,\,/ig))
    str = str.replace(/\,\,/ig, ",");
  while (str.match(/\, /ig))
    str = str.replace(/\, /ig, ",");
...
* Version 2.9
-------------------------------------------------------------------------*/
LeUtilities.aOrB = (a, b) => Math.random() >= 0.5 ? a : b;

LeUtilities.getBezier = (t, C1, C2, C3, C4) => {
  var pos = {x: 0, y: 0};
  var b1 = t * t * t;
  var b2 = 3 * t * t * (1 - t);
  var b3 = 3 * t * (1 - t) * (1 - t);
...
LeUtilities.rotatePoint = (cx, cy, angle, p) => {
 ...
 */
LeUtilities.getCurvePoints = (points, tension, numOfSeg, close) => {
  'use strict';
