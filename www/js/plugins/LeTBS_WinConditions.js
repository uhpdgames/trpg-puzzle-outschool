...
Window_TBSWinConditions.prototype.windowWidth = () => Graphics.width;
...
LeTBSWinCds.getConditionTexts = function () {
    ...
    this._defeatAllEnemies.forEach(enemyId => {
        texts.push(`Defeat all ${$dataEnemies[enemyId].name}s`);
    });
    this._defeatXEnemies.forEach(info => {
        var nbr = info[0];
        var enemyId = info[1];
        if (nbr > 1)
    ...
    this._defeatFlaggedEntities.forEach(info => {
        var flagName = info[1];
        texts.push(`Defeat ${flagName}`);
    });
    this._preventActorsDeath.forEach(actorId => {
        texts.push(`Keep ${$dataActors[actorId].name} alive`);
    });
    this._preventEntitiesDeath.forEach(info => {
        var flagName = info[1];
        texts.push(`Keep ${flagName} alive`);
    });
    this._flagCds.forEach(info => {
        var flagName = info[1];
        texts.push(flagName);
    });
    return texts;
...
LeTBSWinCds.checkVictory = function () {
    ...
    this._defeatAllEnemies.forEach(enemyId => {
        ...
        if (entities.every(ent => ent.battler().isDead())) {
            nbrSuccess++;
        }
    });
    this._defeatXEnemies.forEach(info => {
...
BattleManagerTBS.canPrepareVictory = () => LeTBSWinCds.checkVictory();
