...
TBSEntity.prototype.prepareExtraPopups = function () {
  const sprite = this._sprite || undefined;
//  console.log(sprite);
  if (sprite) {
    if (!sprite._popupsInfo) {
      sprite._popupsInfo = {};
    }
...
Sprite_Damage.prototype.createText = function (text, fontSize = 28, color) {
  var textW = this._damageBitmap.measureTextWidth(text);
  var w = 2 + textW;
  var h = fontSize + 2;
  var sprite = this.createExtraChildSprite(w, h);
...
Sprite_Damage.prototype.createTextIcon = function (text, icL, icR, fontSize = 28, color) {
  var textW = this._damageBitmap.measureTextWidth(text);
...
Sprite_Damage.prototype.drawText = (sprite, text, x, y, maxWidth, align, color) => {
  if (color)
    sprite.bitmap.textColor = color;
  sprite.bitmap.drawText(text, x, y, maxWidth, 28, align);
};

Sprite_Damage.prototype.drawIcon = (sprite, iconIndex, x, y) => {
